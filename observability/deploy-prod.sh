...
# บน prod server ที่มี mosquitto/minio/app รันอยู่แล้วผ่าน compose อื่นบน network เดียวกัน
# ใช้ docker-compose.yml ตัวเดียวกับ deploy.sh (dev) — ต่างกันแค่ env file ที่ชี้ไปยัง
# MinIO/network ของ prod แทน dev (ค่า MinIO ต้องมีอยู่แล้วบน prod server)
#
#   cp observability/.env.prod.example observability/.env.prod   # แก้ค่าจริงก่อนรันครั้งแรก
#   bash observability/deploy-prod.sh
#
...
if [[ ! -f "$ENV_FILE" ]]; then
  echo "ไม่พบ $ENV_FILE — copy จาก .env.prod.example แล้วแก้ค่าก่อน" >&2
  exit 1
fi
ok "พบ $ENV_FILE"
...
if [[ "$FAIL" -eq 0 ]]; then
  cat <<'EOF'
Grafana: http://<prod-server-ip>:3001
